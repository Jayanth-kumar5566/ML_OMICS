...
sink("Documents/MS/ML_OMICS/SNF_standard/Cluster_eval/post_hoc.txt") 
...
if (t$p.value <= 0.05){
  print(i)
  #d=dunn.test(x=list(p1,p2),method = 'bh',alpha=0.05,kw=FALSE,list = TRUE,table = FALSE,altp = TRUE)
...
sink("Documents/MS/ML_OMICS/SNF_standard/Cluster_eval/numerical.txt")
#Numerical
for (i in nam[1:8])
{
  p1=pop1[[i]]
...
sink()
#---------------------Categorical--------------------------
sink("Documents/MS/ML_OMICS/SNF_standard/Cluster_eval/categ_toprocess.txt")
