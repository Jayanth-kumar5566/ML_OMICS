...
sink("Documents/MS/ML_OMICS/SNF_4/Cluster_eval/Demographics/post_hoc.txt") 
...
sink("Documents/MS/ML_OMICS/SNF_4/Cluster_eval/Demographics/numerical.txt")
#Numerical
for (i in nam[1:2])
{
  p1=pop1[[i]]
...
sink()
#---------------------Categorical--------------------------
sink("Documents/MS/ML_OMICS/SNF_4/Cluster_eval/Demographics/categ_toprocess.txt")
for (i in nam[4:5]){
  pop1[i]<-lapply(pop1[i],factor)
  pop2[i]<-lapply(pop2[i],factor)
  pop3[i]<-lapply(pop3[i],factor)
...
#Smoking and ethinicity to do manually